...
const getChatByUser = async (id) => {
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_LOCAL}/chat/${id}`
  );
...
const ChatToAdmin = () => {
  ...
  const { data: dataChat } = useQuery({
    queryKey: ["chat"],
    queryFn: () => getChatByLaporanIdUserId(id, userId),
    refetchInterval: false,
  });
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: () => sendChat(id, userId, message),
    onSuccess: () => {
      setMessage("");
    },
  });
